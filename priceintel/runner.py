...
def _resolve_verdict(st, own_price, market_sources, suspicious_count, cfg, log, has_exact_match=True):
    ...
    if not has_exact_match and base_verdict in ("🔥 РЕКЛАМУВАТИ", "🟢 ПЕРСПЕКТИВНИЙ"):
        return "🟡 ТЕСТУВАТИ", "HIGH_ONLY_MATCH_NOT_EXACT"
...
def run_analysis(input_csv, output_csv, offers_csv, limit=30, marketplaces=None,
    ...
    searcher = HybridSearch(
        ...
        serper_supports_site=cfg.get("serper_supports_site_operator", False),
    )
    log(
        "PROVIDERS: "
    ...
    for i, row in enumerate(rows[start_index:], start_index + 1):
        ...
        for mp in market_cfg:
            ...
            for n, hit in enumerate(hits, 1):
                ...
                verify_domains = set(cfg.get("firecrawl_verification_domains", []))
                if verify_domains and mp["domain"] not in verify_domains:
                    needs_verify = False
        ...
        for n, hit in enumerate(other_hits, 1):
            ...
            verify_domains = set(cfg.get("firecrawl_verification_domains", []))
            if verify_domains and host not in verify_domains:
                needs_verify = False
            if needs_verify and cfg.get("firecrawl_page_verification_enabled", True):
                verified = searcher.verify_page(url)
                if verified:
        ...
        has_exact_match = any(o.get("match_status") == "EXACT" for o in (accepted + accepted_other))
