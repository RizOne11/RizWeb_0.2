...
def _resolve_verdict(st, own_price, market_sources, suspicious_count, cfg, log):
    """Return a final verdict and an explicit reason.

    Safety rules are applied after market statistics are calculated, so the
    final verdict cannot be silently overwritten later in the pipeline.
    """
    base_verdict = st.get("verdict") or "⚪ НЕ ЗНАЙДЕНО"

    # No validated market offers.
    ...
    # Hard protection: own price materially above a market confirmed by at
    # least two independent sources => do not advertise.
    try:
        own_num = float(own_price) if own_price not in (None, "", 0) else None
        median_num = float(st.get("median")) if st.get("median") not in (None, "", 0) else None
        threshold = float(cfg.get("market_overprice_red_pct", 10.0))
        if own_num is not None and median_num and market_sources >= 2:
            over_market_pct = ((own_num - median_num) / median_num) * 100.0
            if over_market_pct >= threshold:
                st["price_score"] = min(int(st.get("price_score") or 0), 39)
                return "🔴 НЕ РЕКЛАМУВАТИ", f"OWN_PRICE_{over_market_pct:.1f}%_ABOVE_MARKET"
    except (TypeError, ValueError, ZeroDivisionError):
        pass

    # One source is useful for orientation but not enough for the strongest
    # recommendation.
...
def run_analysis(input_csv, output_csv, offers_csv, limit=30, marketplaces=None,
    ...
    log(f"ENGINE v0.8.2 | START products={len(rows)} marketplaces={[m['name'] for m in market_cfg]}")
