...
def _serper_safe_query(query, strip_site=True):
    """Normalize queries for Serper compatibility.

    Free accounts may reject quoted phrases, site: operators and negative quoted
    exclusions. Paid accounts support site: normally, so callers with a paid
    plan pass strip_site=False to keep real per-domain targeting instead of
    relying on local post-filtering only.
    """
...
class SerperSearch:
    ...
    def __init__(self, api_key=None, logger=None, gl="ua", hl="uk", num=50, cache=None, cache_ttl=259200,
        ...
        # Paid Serper plans support the site: operator normally. When true,
        # search_all() keeps site:<domain> in the outgoing query instead of
        # stripping it, so each marketplace gets a genuinely targeted query
        # instead of 8 copies of the same bare-keyword search.
        self.supports_site_operator = bool(supports_site_operator)
    ...
    def search_all(self, query, domains, limit_per_domain=5, exact_query=False, site_domain=None):
        if not self.api_key:
            return _empty_grouped(domains)

        raw_query = " ".join(str(query or "").split()).strip() if exact_query else self._extract_identifier(query)
        use_site = bool(self.supports_site_operator and site_domain)
        if use_site:
            raw_query = f"{raw_query} site:{site_domain}"
        simple_query = _serper_safe_query(raw_query, strip_site=not use_site)
        if not simple_query:
            return _empty_grouped(domains)
...
class HybridSearch:
    ...
    def __init__(self, logger=None, gl="ua", hl="uk", num=50, cache=None, cache_ttl=259200,
        ...
        self.serper = SerperSearch(logger=self.log, gl=gl, hl=hl, num=num, cache=cache, cache_ttl=cache_ttl,
                                    supports_site_operator=serper_supports_site)
    ...
    def search_all(self, query, domains, limit_per_domain=20, exact_query=False):
        ...
        # 3) Serper is final recovery only. On free accounts site: is stripped
        # and target-domain filtering happens locally instead. On paid
        # accounts (self.serper.supports_site_operator) we pass the real
        # site_domain through so each marketplace gets a genuinely targeted
        # query instead of 8 copies of the same bare-keyword search.
        if self.serper_enabled and self.serper.api_key and candidate_count() < self.recovery_min_hits:
            sp = self.serper.search_all(
                clean_query, target_domains, limit_per_domain, exact_query=True, site_domain=site_domain
            )
