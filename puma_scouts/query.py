...
def extract_identifiers(mission:ProductMission)->list[str]:
    ...
    for key in _ID_KEYS:
        ...
        if value and value.casefold()!=mission.article.casefold() and value not in found:found.append(value)
    ...
    for token in re.findall(r"\b(?=[A-ZА-ЯІЇЄ0-9-]{4,}\b)(?=[A-ZА-ЯІЇЄ0-9-]*\d)[A-ZА-ЯІЇЄ0-9-]+\b",corpus.upper()):
        if token.casefold()!=mission.article.casefold() and token not in found:found.append(token)
    return found[:12]
def generate_queries(mission:ProductMission)->list[str]:
    ...
    def add(value):
        value=_clean(value)
        if value and value.casefold()!=mission.article.casefold() and value.casefold() not in {q.casefold() for q in queries}:queries.append(value)
    # Restore the previously working discovery order: marketplace title first, then public identifiers.
    # Supplier article remains SKU Lock only and is explicitly excluded above.
    add(name)
