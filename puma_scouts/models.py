...
class ValidatedOffer(BaseModel):
    offer:Offer; verdict:Verdict; score:float=Field(ge=0,le=1); positive_evidence:list[str]=Field(default_factory=list); conflicts:list[str]=Field(default_factory=list); rejection_reasons:list[str]=Field(default_factory=list)
